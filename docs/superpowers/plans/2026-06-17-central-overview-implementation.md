...
**Goal:** 将现有通用仪表盘 CentralOverview.tsx 重写为以设备监控为核心的数据总览页面

**架构:** 单文件组件（遵循现有代码模式），内部拆分 StatCard / DeviceTable / DeviceDetailDialog 等子组件，所有 mock 数据内联生成

**Tech Stack:** React 18 + TypeScript + MUI v7 + Tailwind CSS 4 + Recharts

**设计文档:** `docs/superpowers/specs/2026-06-17-central-overview-design.md`

---

### Task 1: 定义类型和 mock 数据生成

**Files:**
- Rewrite: `src/app/components/CentralOverview.tsx`

- [ ] **Step 1: 替换原有 import 并定义完整类型**

在 `CentralOverview.tsx` 顶部，替换原有的 import 语句，并添加所有新类型定义：
...
- [ ] **Step 2: 编写 mock 数据生成函数**

在类型定义之后，添加仿真设备数据生成代码：
...
const CPU_MODELS = [
  ...
  'Intel Core i5-12400', 'Intel Core i3-10100',
  'AMD Ryzen 5 5600G', 'Intel Celeron N5095',
];
...
const ANOMALY_TYPES = [
  '网络断开', 'CPU 过热', '内存不足', '磁盘空间不足',
  '系统崩溃', '外设异常', '进程无响应',
];

const SOFTWARE_NAMES = [
  '电子白板', '教学助手', '课堂互动', '作业系统',
  '资源平台', '考试系统', 'Office 办公套件', '浏览器',
];
  ...
  for (const building of BUILDINGS) {
    for (const floor of building.floors) {
      for (const room of floor.rooms) {
        ...
        const anomalies: AnomalyRecord[] = Array.from({ length: anomalyCount }, (_, i) => {
          const t = new Date(Date.now() - rand(1, 30) * 86400000);
          return {
            time: formatDate(t),
            type: pick(ANOMALY_TYPES),
            desc: `${pick(ANOMALY_TYPES)}异常`,
            status: Math.random() < 0.7 ? 'resolved' : Math.random() < 0.5 ? 'processing' : 'unresolved',
          } as AnomalyRecord;
        });

        devices.push({
          id: `dev-${idx}`,
          name: `${building.name}${room}教室终端`,
          code: `DEV-EDU-${String(idx).padStart(3, '0')}`,
          building: building.name,
          floor: floor.name,
          room: `${building.name} / ${floor.name} / ${room}教室`,
          status,
          lastActive: isOnline ? formatDate(lastActive) : formatDate(new Date(lastActive.getTime() - 86400000)),
          networkOk,
          hardwareOk,
          smoothnessOk,
          securityOk,
          uploadSpeed: randFloat(0.5, 5),
          downloadSpeed: randFloat(5, 100),
          inboundTraffic: randFloat(50, 500),
          outboundTraffic: randFloat(10, 200),
          hardware: {
            cpuModel: pick(CPU_MODELS),
            cpuUsage,
            cpuTemp,
            memorySize: pick([4, 8, 16]),
            memoryUsage: memUsage,
            systemDiskSize: pick([128, 256]),
            systemDiskUsage: sysDiskUsage,
            dataDiskSize: pick([256, 512]),
            dataDiskUsage,
            os: pick(OS_LIST),
            resolution: pick(RESOLUTIONS),
            touchScreen: Math.random() < 0.9,
            speakerOk: Math.random() < 0.95,
            microphoneOk: Math.random() < 0.90,
            cameraOk: Math.random() < 0.85,
          },
          security: [
...
Run: `npx tsc --noEmit`
Expected: 无类型错误（注意确保所有导入的图标存在）

- [ ] **Step 4: Commit**

```bash
git add src/app/components/CentralOverview.tsx
git commit -m "feat(central-overview): add types and mock data definitions"
```

---

### Task 2: 实现 StatCard 概览卡片和顶部卡片区

- [ ] **Step 1: 添加 StatCard 组件和阈值类型**

在 mock 数据函数之后，添加 StatCard 组件：
...
- [ ] **Step 3: 在主组件中渲染 6 个概览卡片**

在主组件 `CentralOverview` 中，先添加状态和初始化，然后渲染顶部卡片区：
...
```bash
git add src/app/components/CentralOverview.tsx
git commit -m "feat(central-overview): add StatCard and overview cards section"
```
...
- [ ] **Step 1: 添加 ThresholdDialog 组件**

在 StatCard 组件之前或之后，添加阈值设置弹窗组件：
...
在概览卡片下方，添加操作栏（阈值设置 + 导出 + 时间维度切换）：

```typescript
        {/* 操作栏 */}
        <Box className="mb-4 flex items-center justify-between flex-wrap gap-3">
          <Box className="flex items-center gap-2">
            <Button
              variant="outlined"
              size="small"
...
- [ ] **Step 4: 添加 useEffect import**（如果需要的话确认已导入）

确保 `useEffect` 已经在第一行 import 中：`import { useState, useMemo, useCallback, useEffect } from 'react';`

- [ ] **Step 5: Commit**

```bash
git add src/app/components/CentralOverview.tsx
git commit -m "feat(central-overview): add threshold settings dialog"
```
...
function exportToCSV(devices: Device[], filename: string) {
  ...
  const rows = devices.map((d) => [
    ...
    d.lastActive,
    d.uploadSpeed, d.downloadSpeed, d.inboundTraffic, d.outboundTraffic,
...
```bash
git add src/app/components/CentralOverview.tsx
git commit -m "feat(central-overview): add CSV export function"
```

---

### Task 5: 实现设备列表表格（含筛选和分页）

- [ ] **Step 1: 添加筛选和分页逻辑**

在主组件 `CentralOverview` 中，在 `stats` 之后添加过滤逻辑：
...
```bash
git add src/app/components/CentralOverview.tsx
git commit -m "feat(central-overview): add device table with search, filter, pagination"
```

---

### Task 6: 实现设备详情弹窗 — Tab 1（设备信息 + 网络）

- [ ] **Step 1: 添加 DeviceDetailDialog 组件和 Tab 1**

在 ThresholdDialog 组件附近添加设备详情弹窗（先实现 Tab 1，后续 Task 添加其他 Tab）：
...
- [ ] **Step 2: 添加 DeviceDetailDialog 骨架和 Tab1**
...
注意：此时 `HardwareTab`、`SecurityTab`、`UsageTab`、`AnomalyTab` 尚未定义，但 JSX 在有条件渲染时不会报错，因为 `tab` 默认从 0 开始，只有 `DeviceInfoTab` 会渲染。为防止 TS 报错，可以先写空壳组件。

添加临时空壳（后续 Task 将填充实际内容）：

```typescript
function HardwareTab({ device }: { device: Device }) { return <Box className="py-4 text-center text-gray-400">硬件参数加载中...</Box>; }
function SecurityTab({ device }: { device: Device }) { return <Box className="py-4 text-center text-gray-400">安全情况加载中...</Box>; }
function UsageTab({ device }: { device: Device }) { return <Box className="py-4 text-center text-gray-400">使用分析加载中...</Box>; }
function AnomalyTab({ device }: { device: Device }) { return <Box className="py-4 text-center text-gray-400">异常记录加载中...</Box>; }
```

- [ ] **Step 3: 在主组件底部渲染 DeviceDetailDialog**

在 `ThresholdDialog` 之后添加：
...
- [ ] **Step 4: 添加 Close 图标的 import**

确保已导入 `Close` 图标：`Close` 来自 `@mui/icons-material`

- [ ] **Step 5: Commit**

```bash
git add src/app/components/CentralOverview.tsx
git commit -m "feat(central-overview): add device detail dialog with device info tab"
```

---

### Task 7: 实现 Tab 2 — 硬件参数

- [ ] **Step 1: 替换 HardwareTab 空壳为完整实现**
...
```bash
git add src/app/components/CentralOverview.tsx
git commit -m "feat(central-overview): add hardware parameters tab"
```

---

### Task 8: 实现 Tab 3 — 安全情况

- [ ] **Step 1: 替换 SecurityTab 空壳为完整实现**
...
```bash
git add src/app/components/CentralOverview.tsx
git commit -m "feat(central-overview): add security tab"
```

---

### Task 9: 实现 Tab 4 — 使用分析（含迷你图表）

- [ ] **Step 1: 替换 UsageTab 空壳为完整实现**
...
注意：`UsageTab` 中使用了 `Paper`，确保已 import：`import { ..., Paper } from '@mui/material';`

- [ ] **Step 2: Commit**

```bash
git add src/app/components/CentralOverview.tsx
git commit -m "feat(central-overview): add usage analysis tab with charts"
```

---

### Task 10: 实现 Tab 5 — 异常记录

- [ ] **Step 1: 替换 AnomalyTab 空壳为完整实现**
...
```bash
git add src/app/components/CentralOverview.tsx
git commit -m "feat(central-overview): add anomaly records tab"
```
...
- [ ] **Step 2: 启动 dev server 检查渲染**

Run: `npm run dev`
Expected: 集控总览页面显示完整的 6 卡片 + 操作栏 + 设备列表表格，点击"查看详情"弹窗显示 5 个 Tab

- [ ] **Step 3: 验证功能互操作性**
- 点击阈值设置 → 修改值 → 保存 → 卡片颜色变化
- 点击导出数据 → 下载 CSV 文件
- 搜索设备名称 → 表格过滤
- 切换状态/达标筛选 → 表格过滤
- 切换分页 → 正常翻页
- 点击查看详情 → 弹窗打开 → 切换 5 个 Tab → 内容正确
- 关闭弹窗 → 回到列表
...
```bash
git add src/app/components/CentralOverview.tsx
git commit -m "feat(central-overview): complete rewrite with device monitoring dashboard"
```
